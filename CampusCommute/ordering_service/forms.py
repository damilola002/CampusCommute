...
class RideOrderForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'description':      forms.Textarea(attrs={'rows': 3, 'placeholder': 'Any notes for riders...'}),
        }
...
class OfferRideForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            'pickup_location', 'dropoff_location',
            'pickup_lat', 'pickup_lng',
            'approx_time', 'offered_seats', 'description',
        ]
        widgets = {
            'pickup_lat':  forms.HiddenInput(),
            'pickup_lng':  forms.HiddenInput(),
            ...
            'pickup_location':  forms.TextInput(attrs={'placeholder': 'e.g. North Campus Gate'}),
            'dropoff_location': forms.TextInput(attrs={'placeholder': 'e.g. Student Union'}),
            'offered_seats':    forms.NumberInput(attrs={'min': 1, 'max': 8}),
            'description':      forms.Textarea(attrs={'rows': 3, 'placeholder': 'Any notes for riders...'}),
        }
